...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [AcE] %(levelname)s: %(message)s",
    force=True
)
    ...
    try:
        ...
        <html><body style='font-family:Arial,sans-serif;color:#333;margin:0;padding:0'>
          <div style='max-width:600px;margin:32px auto;border:1px solid #ddd;
            ...
            <div style='background:#f9fafb;padding:14px 28px;
              ...
              Automatically generated by AcE — Aeversa WhatsApp Support Bot
            </div>
          </div>
        </body></html>
        """
        ...
        response = requests.post(
            ...
            json={
                "from": FROM_EMAIL,
                "to": [SUPPORT_EMAIL],
                "subject": f"🔴 AcE Escalation — {fault_type or 'Fault'} | {site or 'Unknown site'} | {timestamp}",
                "html": html_body
            },
            timeout=15
        )
...
def handle_agent_command(sender: str, command: str) -> str:
    """Processes commands sent by agents to control AcE."""
...
GREETING = (
    ...
    "My name is *AcE* and I am here to get you charged up. ⚡\n\n"
...
AE_SYSTEM_PROMPT = f"""You are AcE, the WhatsApp support assistant for Aeversa (PTY) Ltd, a South African EV fleet charge point operator.
...
def lookup_charger_and_respond(user_id: str, state: dict,
    ...
    # Alerts are only checked when the charger is confirmed online — for
    # V1 this is a deliberate simplification: if a charger is offline,
    # there's little a customer can actually do about it themselves even
    # knowing which alert is behind it (unlike the online+emergency-stop
    # case, where they can physically go release the button). Smarter
    # offline+alert handling is a V2 idea, not built here yet.
