...
# ── Logging setup (unbuffered, shows in Render logs) ──────────────────────────
logging.basicConfig(
    level=logging.INFO,
...
SMTP_EMAIL    = os.environ.get("SMTP_EMAIL", "")      # Gmail address sending the email
SMTP_PASSWORD = os.environ.get("SMTP_PASSWORD", "")   # Gmail App Password
SUPPORT_EMAIL = "support@aeversa.com"                 # Where escalation emails go
...
def _send_email_worker(customer_number: str, fault_type: str,
    ...
    """Runs in a background thread — sends the escalation email."""
    try:
        log.info(f"Sending escalation email for {customer_number} | {fault_type} | {site}")
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        rows = [
            ("📱 Customer WhatsApp", customer_number),
            ("🕐 Time", timestamp),
            ("⚠️  Fault Type", fault_type),
        ]
        ...
        # Plain text
        text_lines = ["AE SUPPORT ESCALATION", "=" * 40]
        for label, value in rows:
            text_lines.append(f"{label}: {value}")
        text_lines += ["", "=" * 40,
                       "This escalation was automatically generated by AE.",
                       "Please respond to the customer on WhatsApp as soon as possible."]
        plain_text = "\n".join(text_lines)

        # HTML
        html_rows = "".join(
            f"<tr><td style='padding:8px;background:#f5f5f5;font-weight:bold;width:160px'>{l}</td>"
        ...
        <html><body style='font-family:Arial,sans-serif;color:#333'>
          <div style='max-width:600px;margin:auto;border:1px solid #ddd;border-radius:8px;overflow:hidden'>
            <div style='background:#1a1a2e;padding:16px 24px'>
              <h2 style='color:#fff;margin:0'>⚡ AE Support Escalation</h2>
              <p style='color:#aaa;margin:4px 0 0'>Aeversa (PTY) Ltd — Automated Alert</p>
            </div>
            <div style='padding:24px'>
              <p style='margin-top:0'>A customer requires support agent assistance.</p>
        ...
        msg["Subject"] = f"🔴 AE Escalation — {fault_type} | {site or 'Unknown site'} | {timestamp}"
        msg["From"]    = f"AE Support Bot <{SMTP_EMAIL}>"
        ...
        log.info(f"✅ Escalation email sent successfully to {SUPPORT_EMAIL}")
...
def send_escalation_email(customer_number: str, fault_type: str,
    ...
    if not SMTP_EMAIL or not SMTP_PASSWORD:
        log.warning("Email not configured — SMTP_EMAIL or SMTP_PASSWORD missing")
        return
...
def handle_message(user_id: str, msg_raw: str) -> str:
    ...
    if step == "start":
        if msg == "1":
            user_states[user_id] = {"step": "opt1_key_removed", "fault_type": "Vehicle not charging"}
            return (
            ...
            user_states[user_id] = {"step": "opt2_power_on_site", "fault_type": "Charger is off"}
            return (
                "⚫ *Charger is Off*\n\n"
                "Let's investigate! 🔍\n\n"
            ...
            user_states[user_id] = {"step": "opt3_restart_session", "fault_type": "Slow charging"}
            return (
                "🐢 *Slow Charging*\n\n"
...
def webhook():
    ...
    response_text = handle_message(sender, incoming)
...
def health_check():
    ...
    email_status = "configured" if SMTP_EMAIL else "NOT configured"
